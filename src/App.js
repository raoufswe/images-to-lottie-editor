...
function App() {
  ...
  const onMoveAsset = ({ x, y }) => {
    const newLottieFile = moveAsset(lottieFile, selectedLayer, { x, y });
    setLottieFile(newLottieFile);
  };
  ...
  const onLayerSelect = (layer) => {
    ...
    setConfigs({
      ...
      ...(selectedLayer
        ? { x: selectedLayer.ks.a.k[0], y: selectedLayer.ks.a.k[1] }
        : {}),
      ...
      >
        ...
        {lottieFile.layers.map((layer) => (
          ...
          >
            ...
            <IconButton
              icon={<CloseIcon />}
              onClick={onLayerDelete.bind(null, layer)}
              size="sm"
              ml="auto"
            />
          </Flex>
